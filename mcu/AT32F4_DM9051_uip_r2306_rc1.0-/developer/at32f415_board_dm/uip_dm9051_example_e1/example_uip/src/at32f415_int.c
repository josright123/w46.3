...
extern uint32_t g_RunTime;  /*__IO */ //'in ethernetif.c' //JJ-Comp
...
void TMR5_GLOBAL_IRQHandler(void) //Accept TMR5, NOT startup_at32f415.s ; TMR6 (Porting by JJ, as 'TMR6')
{
	...
	//
  lwip_sys_now++; //jj-temp-extern (to compare to 'all_local_time' in "netconf.c")
  ...
  if(tmr_flag_get(TMR5, TMR_OVF_FLAG) != RESET) //TMR6
  {
	...
	/* Update the all_local_time by adding 'SYSTEMTICK_PERIOD_MS10' each SysTick interrupt */
    time_update();
