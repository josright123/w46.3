...
extern unsigned int elapsed_ms;
...
void doState(PtpClock *ptpClock)
{
    ...
    case PTP_MASTER:
		...
		expr = timerExpired(SYNC_INTERVAL_TIMER, ptpClock->itimer);
#if 0
if (expr || ptpClock->itimer[ANNOUNCE_INTERVAL_TIMER].expire) {
	printf("elapsed_ms %lu\r\n", elapsed_ms);
	printf("sync [intvl %5lu] [left %5lu] [expire %d]\r\n",
		ptpClock->itimer[SYNC_INTERVAL_TIMER].interval,
		ptpClock->itimer[SYNC_INTERVAL_TIMER].left,
		expr);
	printf("annu [intvl %5lu] [left %5lu] [expire %d]\r\n",
		ptpClock->itimer[ANNOUNCE_INTERVAL_TIMER].interval,
		ptpClock->itimer[ANNOUNCE_INTERVAL_TIMER].left,
		ptpClock->itimer[ANNOUNCE_INTERVAL_TIMER].expire);
}
#endif
        if (expr)
        {
...
#if 1
//			printf("in DELAY_RESP from-master-pk_ts(%u.%09u)\r\n",
//				ptpClock->msgTmp.resp.receiveTimestamp.secondsField.lsb,
//				ptpClock->msgTmp.resp.receiveTimestamp.nanosecondsField);
#endif
            isFromCurrentParent = isSamePortIdentity(
				&ptpClock->parentDS.parentPortIdentity,
				&ptpClock->msgTmpHeader.sourcePortIdentity);
