...
//printf("\r\n");
//printf("%s igmp_leavegroup.s,     IP value 0x%08x\r\n", PTPD_HEADER_MCU_DESC, multicastAaddr.addr);
    igmp_leavegroup(IP_ADDR_ANY, &multicastAaddr);
//printf("%s igmp_leavegroup.e,     IP value 0x%08x\r\n", PTPD_HEADER_MCU_DESC, multicastAaddr.addr);
//printf("\r\n");
...
printf("[PTPD] ptpdaemon,netInit: igmp_joingroup: to encode multi-cast address: %s\r\n", addrStr);
...
printf("[PTPD] netInit: igmp_joingroup: to encode peer multi-cast address: %s\r\n", addrStr);
    igmp_joingroup(&interfaceAddr, (ip_addr_t *)&netAddr);
